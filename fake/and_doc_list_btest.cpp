...
TEST_F(AndDocListBtest, oneTerm) {
  ...
  EXPECT_EQ(1, _andDocList.next(0));
  EXPECT_EQ(3, _andDocList.next(2));
  EXPECT_EQ(3, _andDocList.next(3));
  EXPECT_EQ(5, _andDocList.next(4));
  EXPECT_EQ(DocList::INVALID_DOC_ID, _andDocList.next(6));
}
...
TEST_F(AndDocListBtest, threeTerm) {
  ...
  EXPECT_EQ(30, _andDocList.next(0));
  EXPECT_EQ(60, _andDocList.next(31));
  EXPECT_EQ(DocList::INVALID_DOC_ID, _andDocList.next(61));
}
